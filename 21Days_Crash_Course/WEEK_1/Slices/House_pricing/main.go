...
Istanbul,500,10,5,1000000`
	...
	var sizeSum, bedsSum, bathsSum, priceSum float64
	for j := range rows {
		sizeSum += float64(sizes[j])
		bedsSum += float64(beds[j])
		bathsSum += float64(baths[j])
		priceSum += float64(prices[j])
	}
	fmt.Printf("%-15s", "")
	fmt.Printf("%-15.2f", sizeSum/float64(len(sizes)))
	fmt.Printf("%-15.2f", bedsSum/float64(len(beds)))
	fmt.Printf("%-15.2f", bathsSum/float64(len(baths)))
	fmt.Printf("%-15.2f", priceSum/float64(len(prices)))
